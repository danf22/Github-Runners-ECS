...
# Idle timeout watchdog: terminate if no job is picked up within timeout
_JOBS_ACCEPTANCE_TIMEOUT=${JOBS_ACCEPTANCE_TIMEOUT:-300}
if [[ ${_JOBS_ACCEPTANCE_TIMEOUT} -gt 0 ]] && [[ -n "${EPHEMERAL}" ]]; then
  echo "Idle timeout watchdog enabled: ${_JOBS_ACCEPTANCE_TIMEOUT}s"
  (
...
if [[ ${_RUN_AS_ROOT} == "true" ]]; then
  ...
  if [[ $(id -u) -eq 0 ]]; then
    ...
    # The toolcache is not recursively chowned to avoid recursing over
    # prepulated tooling in derived docker images
